...
lazy val root = (project in file("."))
  .settings(
    ...
    dockerExposedPorts     ++= Seq(8080),
      ...
      dockerCommands           := Seq(Cmd("FROM", "registry.access.redhat.com/ubi9-minimal:latest AS stage0"),
                                      ...
                                      // Build stage for microdnf
                                      Cmd("FROM", "registry.access.redhat.com/ubi9-minimal:latest AS base"),
                                      Cmd("FROM", "registry.access.redhat.com/ubi9-micro:latest AS runtime"),
                                      Cmd("COPY --from=stage0 /usr/bin/microdnf /usr/bin/"),
                                      Cmd("COPY --from=stage0 /usr/bin/gpg /usr/bin/"),
                                      Cmd("COPY --from=stage0 /usr/bin/gpg2 /usr/bin/"),
                                      Cmd("COPY --from=stage0 /lib64 /lib64/"),
                                      Cmd("COPY --from=stage0 /usr/lib64 /usr/lib64/"),
                                      Cmd("COPY --from=stage0 /usr/lib/rpm /usr/lib/rpm/"),
                                      Cmd("COPY --from=stage0 /etc/dnf /etc/dnf/"),
                                      Cmd("COPY --from=stage0 /etc/rpm /etc/rpm/"),
                                      Cmd("COPY --from=stage0 /etc/pki /etc/pki/"),
