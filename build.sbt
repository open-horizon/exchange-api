...
lazy val root = (project in file("."))
  .settings(
    ...
    dockerCommands           := Seq(Cmd("FROM", dockerBaseImage.value ++ " AS stage0"),
                                    ...
                                    Cmd("USER", "root"),
                                    Cmd("RUN", "chmod -R u=r,g=r /licenses && chmod -R u=rX,g=rX /4/opt/docker /2/opt/docker && chmod u+x,g+x /4/opt/docker/bin/" ++ name.value),
                                    ...
                                    Cmd("RUN", "chown -R " ++ (Docker / daemonUser).value ++ ":" ++ (Docker / daemonGroup).value, " /etc/horizon/exchange"),
                                    Cmd("WORKDIR", "/licenses"),
