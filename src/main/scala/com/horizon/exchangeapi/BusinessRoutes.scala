        ...
        complete({
          ...
          db.run(valServiceIdActions.asTry.flatMap({
            ...
            case Success(v) =>
              ...
              val policyChange = ResourceChangeRow(0L, orgid, policy, "policy", "false", "policy", ResourceChangeConfig.CREATED, ApiTime.nowUTC)
          ...
          db.run(valServiceIdActions.asTry.flatMap({
            ...
            case Success(n) =>
              ...
              if (numUpdated > 0) {
                if (owner != "") AuthCache.putBusinessOwner(compositeId, owner) // currently only users are allowed to update business policy resources, so owner should never be blank
                AuthCache.putBusinessIsPublic(compositeId, isPublic = false)
                val policyChange = ResourceChangeRow(0L, orgid, policy, "policy", "false", "policy", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTC)
                policyChange.insert.asTry
              } else {
            ...
            db.run(valServiceIdActions.asTry.flatMap({
              ...
              case Success(n) =>
                ...
                if (numUpdated > 0) { // there were no db errors, but determine if it actually found it or not
                  val policyChange = ResourceChangeRow(0L, orgid, policy, "policy", "false", "policy", ResourceChangeConfig.MODIFIED, ApiTime.nowUTC)
                  policyChange.insert.asTry
                } else {
                  DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("business.policy.not.found", compositeId))).asTry
                }
        ...
        db.run(BusinessPoliciesTQ.getBusinessPolicy(compositeId).delete.transactionally.asTry.flatMap({
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              ...
              val policyChange = ResourceChangeRow(0L, orgid, policy, "policy", "false", "policy", ResourceChangeConfig.DELETED, ApiTime.nowUTC)
              policyChange.insert.asTry
            } else {
              DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("business.policy.not.found", compositeId))).asTry
            }
