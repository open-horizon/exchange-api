...
object ExchangeApiApp extends App {
  ...
  lazy val routes: Route = DebuggingDirectives.logRequestResult(requestResponseLogging _) { pathPrefix("v1") { testRoute ~ orgsRoutes ~ usersRoutes ~ swaggerDocRoutes ~ swaggerUiRoutes } }
