...
class ExchangeApiApp(val db: Database)(implicit val swagger: Swagger) extends ScalatraServlet
    with FutureSupport with NativeJsonSupport with SwaggerSupport with AuthenticationSupport with DevicesRoutes with AgbotsRoutes with UsersRoutes with AdminRoutes with BlockchainsRoutes with MicroserviceRoutes {
