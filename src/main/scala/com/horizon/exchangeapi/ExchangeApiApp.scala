...
class ExchangeApiApp(val db: Database)(implicit val swagger: Swagger) extends ScalatraServlet
    with FutureSupport with NativeJsonSupport with SwaggerSupport with CorsSupport with AuthenticationSupport with NodesRoutes with AgbotsRoutes with UsersRoutes with AdminRoutes with ServiceRoutes with ResourceRoutes with PatternRoutes with OrgRoutes {
