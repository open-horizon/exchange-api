...
class ServiceRoutes(implicit val system: ActorSystem) extends JacksonSupport with AuthenticationSupport {
  ...
  def servicesGetRoute: Route = (get & path("orgs" / Segment / "services") & parameter(('owner.?, 'public.?, 'url.?, 'version.?, 'arch.?, 'requiredurl.?))) { (orgid, owner, public, url, version, arch, requiredurl) =>
  ...
  @Operation(summary = "Returns a service", description = "Returns the service with the specified id. Can be run by a user, node, or agbot.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetRoute: Route = (get & path("orgs" / Segment / "services" / Segment) & parameter(('attribute.?))) { (orgid, service, attribute) =>
...
```""", required = true, content = Array(new Content(schema = new Schema(implementation = classOf[PostPutServiceRequest])))),
        ...
        complete({
          ...
          db.run(svcAction.asTry.flatMap({
            ...
            case Success(v) =>
              ...
              val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", reqBody.public.toString, "service", ResourceChangeConfig.CREATED, ApiTime.nowUTC)
          ...
          db.run(svcAction.asTry.flatMap({
            ...
            case Success(n) =>
              ...
              if (numUpdated > 0) {
                ...
                val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", reqBody.public.toString, "service", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTC)
                serviceChange.insert.asTry
              } else {
  ...
  @Operation(summary = "Updates 1 attribute of a service", description = "Updates one attribute of a service. This can only be called by the user that originally created this service resource.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def servicePatchRoute: Route = (patch & path("orgs" / Segment / "services" / Segment) & entity(as[String]) & entity(as[PatchServiceRequest])) { (orgid, service, reqBodyAsStr, reqBody) =>
  ...
  @Operation(summary = "Deletes a service", description = "Deletes a service. Can only be run by the owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceDeleteRoute: Route = (delete & path("orgs" / Segment / "services" / Segment)) { (orgid, service) =>
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              ...
              val serviceId = compositeId.substring(compositeId.indexOf("/") + 1, compositeId.length)
              val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", storedPublicField.toString, "service", ResourceChangeConfig.DELETED, ApiTime.nowUTC)
              serviceChange.insert.asTry
            } else {
              DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
            }
  ...
  @Operation(summary = "Returns the service policy", description = "Returns the service policy. Can be run by a user, node or agbot.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetPolicyRoute: Route = (get & path("orgs" / Segment / "services" / Segment / "policy")) { (orgid, service) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
...
```""", required = true, content = Array(new Content(schema = new Schema(implementation = classOf[PutServicePolicyRequest])))),
      ...
      complete({
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = compositeId.substring(compositeId.indexOf("/") + 1, compositeId.length)
              val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", storedPublicField.toString, "servicepolicies", ResourceChangeConfig.DELETED, ApiTime.nowUTC)
              serviceChange.insert.asTry
            } else {
  ...
  @Operation(summary = "Returns all keys/certs for this service", description = "Returns all the signing public keys/certs for this service. Can be run by any credentials able to view the service.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetKeysRoute: Route = (get & path("orgs" / Segment / "services" / Segment / "keys")) { (orgid, service) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
  ...
  @Operation(summary = "Returns a key/cert for this service", description = "Returns the signing public key/cert with the specified keyid for this service. The raw content of the key/cert is returned, not json. Can be run by any credentials able to view the service.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetKeyRoute: Route = (get & path("orgs" / Segment / "services" / Segment / "keys" / Segment)) { (orgid, service, keyId) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
  ...
  @Operation(summary = "Adds/updates a key/cert for the service", description = "Adds a new signing public key/cert, or updates an existing key/cert, for this service. This can only be run by the service owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def servicePutKeyRoute: Route = (put & path("orgs" / Segment / "services" / Segment / "keys" / Segment) & entity(as[String])) { (orgid, service, keyId, reqEntity) =>
    val compositeId = OrgAndId(orgid, service).toString
    exchAuth(TService(compositeId),Access.WRITE) { _ =>
      ...
      val reqBody = PutServiceKeyRequest(reqBodyAsRawString)
      validate(reqBody.getAnyProblem.isEmpty, "Problem in request body") {
        complete({
  ...
  @Operation(summary = "Deletes all keys of a service", description = "Deletes all of the current keys/certs for this service. This can only be run by the service owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceDeleteKeysRoute: Route = (delete & path("orgs" / Segment / "services" / Segment / "keys")) { (orgid, service) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId), Access.WRITE) { _ =>
      complete({
        var storedPublicField = false
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = service.substring(service.indexOf("/") + 1, service.length)
              val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", storedPublicField.toString, "servicekeys", ResourceChangeConfig.DELETED, ApiTime.nowUTC)
              serviceChange.insert.asTry
            } else {
  ...
  @Operation(summary = "Deletes a key of a service", description = "Deletes a key/cert for this service. This can only be run by the service owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceDeleteKeyRoute: Route = (delete & path("orgs" / Segment / "services" / Segment / "keys" / Segment)) { (orgid, service, keyId) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId), Access.WRITE) { _ =>
      complete({
        var storedPublicField = false
  ...
  @Operation(summary = "Returns all docker image tokens for this service", description = "Returns all the docker image authentication tokens for this service. Can be run by any credentials able to view the service.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetDockauthsRoute: Route = (get & path("orgs" / Segment / "services" / Segment / "dockauths")) { (orgid, service) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
  ...
  @Operation(summary = "Returns a docker image token for this service", description = "Returns the docker image authentication token with the specified dockauthid for this service. Can be run by any credentials able to view the service.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceGetDockauthRoute: Route = (get & path("orgs" / Segment / "services" / Segment / "dockauths" / Segment)) { (orgid, service, dockauthIdAsStr) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
...
```""", required = true, content = Array(new Content(schema = new Schema(implementation = classOf[PostPutServiceDockAuthRequest])))),
  ...
  @Operation(summary = "Deletes all docker image auth tokens of a service", description = "Deletes all of the current docker image auth tokens for this service. This can only be run by the service owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceDeleteDockauthsRoute: Route = (delete & path("orgs" / Segment / "services" / Segment / "dockauths")) { (orgid, service) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId), Access.WRITE) { _ =>
      complete({
        var storedPublicField = false
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = service.substring(service.indexOf("/") + 1, service.length)
              val serviceChange = ResourceChangeRow(0, orgid, serviceId, "service", storedPublicField.toString, "servicedockauths", ResourceChangeConfig.DELETED, ApiTime.nowUTC)
              serviceChange.insert.asTry
            } else {
  ...
  @Operation(summary = "Deletes a docker image auth token of a service", description = "Deletes a docker image auth token for this service. This can only be run by the service owning user.",
    ...
    responses = Array(
      ...
      new responses.ApiResponse(responseCode = "403", description = "access denied"),
      new responses.ApiResponse(responseCode = "404", description = "not found")))
  def serviceDeleteDockauthRoute: Route = (delete & path("orgs" / Segment / "services" / Segment / "dockauths" / Segment)) { (orgid, service, dockauthIdAsStr) =>
    val compositeId = OrgAndId(orgid,service).toString
    exchAuth(TService(compositeId), Access.WRITE) { _ =>
      complete({
        Try(dockauthIdAsStr.toInt) match {
