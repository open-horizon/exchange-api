        ...
        attribute match {
          ...
          case None =>  // Return the whole service resource
            db.run(ServicesTQ.getService(compositeId).result).map({ list =>
              logger.debug("GET /orgs/" + orgid + "/services result size: " + list.size)
              val services = list.map(e => e.service -> e.toService).toMap
              val code = if (services.nonEmpty) StatusCodes.OK else StatusCodes.NotFound
              (code, GetServicesResponse(services, 0))
            })
        }
    ...
    exchAuth(TService(OrgAndId(orgid,"").toString), Access.CREATE) { ident =>
      validateWithMsg(reqBody.getAnyProblem(orgid, null)) {
        complete({
          ...
          val svcIds = reqBody.requiredServices.getOrElse(List()).map( s => ServicesTQ.formId(s.org, s.url, "%", s.arch) )
          ...
          db.run(svcAction.asTry.flatMap({
            ...
            case Failure(t: org.postgresql.util.PSQLException) => if (t.getMessage.contains("duplicate key value violates unique constraint")) {
              ...
              (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.not.created", service, t.getMessage)))
            }
            case Failure(t) =>
          ...
          db.run(svcAction.asTry.flatMap({
            ...
            case Success(n) =>
              ...
              if (numUpdated > 0) {
                ...
                val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", reqBody.public.toString, "service", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTCTimestamp)
                serviceChange.insert.asTry
              } else {
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              ...
              else (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.not.updated", compositeId, t.getMessage)))
            case Failure(t) =>
              if (t.getMessage.startsWith("Access Denied:")) (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("service.not.updated", compositeId, t.getMessage)))
            ...
            db.run(svcAction.transactionally.asTry.flatMap({
              ...
              case Success(public) =>
                ...
                if (public.nonEmpty) {
                  ...
                  val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", publicField.toString, "service", ResourceChangeConfig.MODIFIED, ApiTime.nowUTCTimestamp)
                  serviceChange.insert.asTry
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              ...
              val serviceId = compositeId.substring(compositeId.indexOf("/") + 1, compositeId.length)
              val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", storedPublicField.toString, "service", ResourceChangeConfig.DELETED, ApiTime.nowUTCTimestamp)
              serviceChange.insert.asTry
            } else {
              DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
            }
          ...
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.not.deleted", compositeId, t.toString)))
          case Failure(t) =>
        ...
        complete({
          db.run(reqBody.toServicePolicyRow(compositeId).upsert.asTry.flatMap({
            ...
            case Success(public) =>
              ...
              if (public.nonEmpty) {
                val serviceId = compositeId.substring(compositeId.indexOf("/") + 1, compositeId.length)
                val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", public.head.toString, "servicepolicies", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTCTimestamp)
                serviceChange.insert.asTry
              } else DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
            case Failure(t) => DBIO.failed(t).asTry
          })).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              ...
              else (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("policy.not.inserted.or.updated", compositeId, t.toString)))
            case Failure(t) =>
              if (t.getMessage.startsWith("Access Denied:")) (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("policy.not.inserted.or.updated", compositeId, t.getMessage)))
      ...
      complete({
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = compositeId.substring(compositeId.indexOf("/") + 1, compositeId.length)
              val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", storedPublicField.toString, "servicepolicies", ResourceChangeConfig.DELETED, ApiTime.nowUTCTimestamp)
              serviceChange.insert.asTry
            } else {
          ...
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.policy.not.deleted", compositeId, t.toString)))
          case Failure(t) =>
          ...
          complete({
            db.run(reqBody.toServiceKeyRow(compositeId, keyId).upsert.asTry.flatMap({
              ...
              case Success(public) =>
                ...
                if (public.nonEmpty) {
                  val serviceId = service.substring(service.indexOf("/") + 1, service.length)
                  val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", public.head.toString, "servicekeys", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTCTimestamp)
                  serviceChange.insert.asTry
                } else DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
              case Failure(t) => DBIO.failed(t).asTry
            })).map({
              ...
              case Failure(t: org.postgresql.util.PSQLException) =>
                ...
                else (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.key.not.inserted.or.updated", keyId, compositeId, t.getMessage)))
              case Failure(t) =>
                if (t.getMessage.startsWith("Access Denied:")) (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("service.key.not.inserted.or.updated", keyId, compositeId, t.getMessage)))
      ...
      complete({
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = service.substring(service.indexOf("/") + 1, service.length)
              val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", storedPublicField.toString, "servicekeys", ResourceChangeConfig.DELETED, ApiTime.nowUTCTimestamp)
              serviceChange.insert.asTry
            } else {
          ...
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.keys.not.deleted", compositeId, t.toString)))
          case Failure(t) =>
      ...
      complete({
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.key.not.deleted", keyId, compositeId, t.toString)))
          case Failure(t) =>
    ...
    exchAuth(TService(compositeId),Access.READ) { _ =>
      complete({
        Try(dockauthIdAsStr.toInt) match {
          ...
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, "dockauthid must be an integer: " + t.getMessage))
          case Failure(t) =>
        ...
        complete({
          ...
          db.run(reqBody.getDupDockAuth(compositeId).result.asTry.flatMap({
            ...
            case Success(public) =>
              ...
              if (public.nonEmpty) {
                val serviceId = service.substring(service.indexOf("/") + 1, service.length)
                val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", public.head.toString, "servicedockauths", ResourceChangeConfig.CREATED, ApiTime.nowUTCTimestamp)
                serviceChange.insert.asTry
              } else DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
            case Failure(t) => DBIO.failed(t).asTry
          })).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              ...
              else (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.dockauth.not.inserted", dockAuthId, compositeId, t.getMessage)))
            case Failure(t) =>
              if (t.getMessage.startsWith("Access Denied:")) (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("service.dockauth.not.inserted", dockAuthId, compositeId, t.getMessage)))
        ...
        complete({
          ...
          db.run(reqBody.toServiceDockAuthRow(compositeId, dockAuthId).update.asTry.flatMap({
            case Success(n) =>
              // Get the value of the public field
              logger.debug("POST /orgs/" + orgid + "/services/" + service + "/dockauths result: " + n)
              val numUpdated = n.asInstanceOf[Int] // n is an AnyRef so we have to do this to get it to an int
            ...
            case Success(public) =>
              ...
              if (public.nonEmpty) {
                val serviceId = service.substring(service.indexOf("/") + 1, service.length)
                val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", public.head.toString, "servicedockauths", ResourceChangeConfig.CREATEDMODIFIED, ApiTime.nowUTCTimestamp)
                serviceChange.insert.asTry
              } else DBIO.failed(new DBProcessingError(HttpCode.NOT_FOUND, ApiRespType.NOT_FOUND, ExchMsg.translate("service.not.found", compositeId))).asTry
            case Failure(t) => DBIO.failed(t).asTry
          })).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              ...
              else (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.dockauth.not.updated", dockAuthId, compositeId, t.getMessage)))
            case Failure(t) =>
              if (t.getMessage.startsWith("Access Denied:")) (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("service.dockauth.not.updated", dockAuthId, compositeId, t.getMessage)))
      ...
      complete({
        ...
        db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
          ...
          case Success(v) =>
            ...
            if (v > 0) { // there were no db errors, but determine if it actually found it or not
              val serviceId = service.substring(service.indexOf("/") + 1, service.length)
              val serviceChange = ResourceChangeRow(0L, orgid, serviceId, "service", storedPublicField.toString, "servicedockauths", ResourceChangeConfig.DELETED, ApiTime.nowUTCTimestamp)
              serviceChange.insert.asTry
            } else {
          ...
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.dockauths.not.deleted", compositeId, t.toString)))
          case Failure(t) =>
          ...
          case Success(dockauthId) =>
            ...
            db.run(ServicesTQ.getPublic(compositeId).result.asTry.flatMap({
              ...
              case Failure(t: DBProcessingError) =>
                t.toComplete
              case Failure(t: org.postgresql.util.PSQLException) =>
                (HttpCode.BAD_GW, ApiResponse(ApiRespType.BAD_GW, ExchMsg.translate("service.dockauths.not.deleted", dockauthId, compositeId, t.toString)))
              case Failure(t) =>
