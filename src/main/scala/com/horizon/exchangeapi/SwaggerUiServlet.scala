...
class SwaggerUiServlet extends ScalatraServlet {
  ...
  def redirectToOurSwagger = {
    ...
    request.header("X-API-Request") match {
      ...
      case Some(url) => redirect(url+"/api?url="+url+"/api-docs")
      ...
      case None => val R1 = "^(.*)/api$".r
        ...
        request.uri.toString match {
          case R1(first) => redirect(first+"/api?url="+first+"/api-docs")
          case R2(first,second) => redirect(first+"/"+second+"?url="+first+"/api-docs")
