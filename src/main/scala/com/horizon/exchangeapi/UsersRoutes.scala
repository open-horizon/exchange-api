        ...
        complete({
          ...
          db.run(UserRow(compositeId, orgid, hashedPw, reqBody.admin, reqBody.email, ApiTime.nowUTC, updatedBy).insertUser().asTry).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              (HttpCode.SERVICE_UNAVAILABLE, ApiResponse(ApiRespType.SERVICE_UNAVAILABLE, ExchMsg.translate("user.not.added", t.toString)))
            case Failure(t) =>
      ...
      validateWithMsg(reqBody.getAnyProblem(ident.isAdmin)) {
        complete({
          ...
          db.run(UserRow(compositeId, orgid, hashedPw, reqBody.admin, reqBody.email, ApiTime.nowUTC, updatedBy).updateUser().asTry).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              (HttpCode.SERVICE_UNAVAILABLE, ApiResponse(ApiRespType.SERVICE_UNAVAILABLE, ExchMsg.translate("user.not.updated", t.toString)))
            case Failure(t) =>
    ...
    exchAuth(TUser(compositeId), Access.WRITE) { _ =>
      complete({
        ...
        db.run(UsersTQ.getUser(compositeId).delete.transactionally.asTry).map({
          ...
          case Failure(t: org.postgresql.util.PSQLException) =>
            (HttpCode.SERVICE_UNAVAILABLE, ApiResponse(ApiRespType.SERVICE_UNAVAILABLE, ExchMsg.translate("user.not.deleted", compositeId, t.toString)))
          case Failure(t) =>
      ...
      validateWithMsg(reqBody.getAnyProblem) {
        complete({
          ...
          db.run(action.transactionally.asTry).map({
            ...
            case Failure(t: org.postgresql.util.PSQLException) =>
              (HttpCode.SERVICE_UNAVAILABLE, ApiResponse(ApiRespType.SERVICE_UNAVAILABLE, ExchMsg.translate("user.password.not.updated", compositeId, t.toString)))
            case Failure(t) =>
