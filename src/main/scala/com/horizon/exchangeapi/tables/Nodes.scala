...
final case class RegService(url: String, numAgreements: Int, configState: Option[String], policy: String, properties: List[Prop], version: String) extends RegServiceTrait
...
final case class NodeRow(id: String, 
  ...
  def toNode(superUser: Boolean): Node = {
    ...
    val rsvc2: List[RegService] = rsvc.map(rs => RegService(rs.url, rs.numAgreements, rs.configState.orElse(Some("active")), rs.policy, rs.properties, rs.version))
