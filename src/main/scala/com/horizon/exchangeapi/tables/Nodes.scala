...
case class NodeRow(id: String, orgid: String, token: String, name: String, owner: String, pattern: String, regServices: String, userInput: String, msgEndPoint: String, softwareVersions: String, lastHeartbeat: String, publicKey: String, arch: String) {
...
case class Prop(name: String, value: String, propType: String, op: String) {
  ...
  def validate: Option[String] = {
    ...
    if (propType==PropType.BOOLEAN) {
      if (op!=Op.EQUAL) return Option[String](Messages("invalid.op.for.name.opequal", op, name, Op.EQUAL, PropType.BOOLEAN))
      if (value.toLowerCase != "true" && value.toLowerCase != "false" && value != "*") return Option[String](Messages("invalid.boolean.value.for.name", value, name))
    }
    if ((propType==PropType.LIST || propType==PropType.STRING) && op!=Op.IN) return Option[String](Messages("invalid.op.for.name.proplist", op, name, Op.IN, PropType.STRING, PropType.LIST))
    if (propType==PropType.INT) {
      if (op==Op.IN) return Option[String](Messages("invalid.op.for.name", op, name))
      //      if (op==Op.IN) return Option[String]("invalid op '"+op+"' specified for "+name)
      if (value != "*") {
        // ensure its a valid integer number
        try { value.toInt }
        catch { case _: Exception => return Option[String](Messages("invalid.int.for.name", value, name)) }
      }
    }
    if (propType==PropType.VERSION) {
      if (!(op==Op.EQUAL || op==Op.IN)) return Option[String](Messages("invalid.op.for.name.propversion", op, name, Op.EQUAL, Op.IN, PropType.VERSION))
      if (value != "*") {       // verify it is a valid version or range format
        if (!VersionRange(value).isValid) return Option[String](Messages("invalid.version.for.name", value, name))
      }
    }
    return None
  }
