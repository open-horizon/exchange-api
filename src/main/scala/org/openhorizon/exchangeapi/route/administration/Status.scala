...
trait Status extends JacksonSupport with AuthenticationSupport {
  ...
  def getStatus(identity: Identity, organization: Option[String] = None): Route = {
    ...
    complete({
      ...
      db.run(something.transactionally.asTry).map {
        case Success(result) =>
          if (result._9 == 0) // Organization not found
            (HttpCode.NOT_FOUND, ApiResponse(ApiRespType.NOT_FOUND, ExchMsg.translate("org.not.found", organization.getOrElse(""))))
          else
            (HttpCode.OK, new AdminStatus(DBMetrics = result, message = ExchMsg.translate("exchange.server.operating.normally")))
        case Failure(t: org.postgresql.util.PSQLException) =>
          if (t.getMessage.contains("An I/O error occurred while sending to the backend"))
            (HttpCode.BAD_GW, AdminStatus)
          else
            (HttpCode.INTERNAL_ERROR, AdminStatus)
        case Failure(t) =>
          (HttpCode.INTERNAL_ERROR, AdminStatus(msg = t.getMessage))
      }
    })
  }
