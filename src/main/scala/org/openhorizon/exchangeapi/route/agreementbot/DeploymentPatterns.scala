...
import akka.http.scaladsl.server.Directives.{as, complete, delete, entity, get, path, post, _}
...
import org.openhorizon.exchangeapi.{Access, ApiRespType, ApiResponse, AuthenticationSupport, ExchMsg, ExchangePosgtresErrorHandling, HttpCode, OrgAndId, TAgbot, auth}
                ...
                db.run(PatternsTQ.getPattern(OrgAndId(reqBody.patternOrgid, reqBody.pattern).toString).length.result.asTry
                                 ...
                                 .flatMap({
                                   case Success(v) => // Add the resource to the resourcechanges table
                                     ...
                                     ResourceChange(0L, organization, agreementBot, ResChangeCategory.AGBOT, false, ResChangeResource.AGBOTPATTERNS, ResChangeOperation.CREATED).insert.asTry
                                   case Failure(t) =>
                                     DBIO.failed(t).asTry}))
                  .map({
                    case Success(v) =>
