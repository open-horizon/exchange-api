...
import org.openhorizon.exchangeapi.table.{AgbotBusinessPol, AgbotBusinessPolsTQ, BusinessPoliciesTQ, ResChangeCategory, ResChangeOperation, ResChangeResource, ResourceChange}
                                  ...
                                  .flatMap({
                                    case Success(v) =>
                                      if (v > 0) {// there were no db errors, but determine if it actually found it or not
                                        ...
                                        ResourceChange(0L, organization, agreementBot, ResChangeCategory.AGBOT, public = false, ResChangeResource.AGBOTBUSINESSPOLS, ResChangeOperation.DELETED).insert.asTry
                                      }
                                      else
              ...
              db.run(BusinessPoliciesTQ.getBusinessPolicy(OrgAndId(reqBody.businessPolOrgid, reqBody.businessPol).toString)
                                       ...
                                       .flatMap({
                                         case Success(v) => // Add the resource to the resourcechanges table
                                           ...
                                           ResourceChange(0L, organization, agreementBot, ResChangeCategory.AGBOT, public = false, ResChangeResource.AGBOTBUSINESSPOLS, ResChangeOperation.CREATED).insert.asTry
                                         case Failure(t) =>
                                           DBIO.failed(t).asTry}))
                .map({
                  case Success(v) =>
