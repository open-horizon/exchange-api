...
trait DeploymentPatterns extends JacksonSupport with AuthenticationSupport {
  ...
  def deleteDeploymentPatterns(@Parameter(hidden = true) agreementBot: String,
    ...
    delete {
      Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")})") }
      ...
      complete({ // remove does *not* throw an exception if the key does not exist
        ...
        db.run(AgbotPatternsTQ.getPatterns(resource)
                              ...
                              .flatMap({
                                case Success(v) =>
                                  if (v > 0) { // there were no db errors, but determine if it actually found it or not
                                    ...
                                    Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Managed Deployment Patterns Deleted: ${v}") }
          ...
          .map({
            case Success(v) =>
              Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Changes Logged:                      ${v}") }
              (HttpCode.DELETED, ApiResponse(ApiRespType.OK, ExchMsg.translate("patterns.deleted")))
            case Failure(exception: DBProcessingError) =>
              Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${Serialization.write(exception.toComplete)}") }
              exception.toComplete
            case Failure(exception: org.postgresql.util.PSQLException) =>
              Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${Serialization.write(ExchangePosgtresErrorHandling.ioProblemError(exception, ExchMsg.translate("patterns.not.deleted", resource, exception.toString)))}") }
            ...
            case Failure(exception) =>
              Future { logger.debug(s"DELETE /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${(HttpCode.INTERNAL_ERROR, Serialization.write(ApiResponse(ApiRespType.INTERNAL_ERROR, ExchMsg.translate("patterns.not.deleted", resource, exception.toString))))}") }
              (HttpCode.INTERNAL_ERROR, ApiResponse(ApiRespType.INTERNAL_ERROR, ExchMsg.translate("patterns.not.deleted", resource, exception.toString)))
          })
        }) // end of complete
    }
  ...
  def getDeploymentPatterns(@Parameter(hidden = true) agreementBot: String,
    ...
    parameter("deployment_pattern".?,
      ...
      (deploymentPattern,
       ...
       nodeOrganization) =>
        Future { logger.debug(s"GET /orgs/${organization}/agbots/${agreementBot}/patterns?deployment_pattern=${deploymentPattern}&deployment_pattern_organization=${deploymentPatternOrganization}&node_organization=${nodeOrganization} - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")})") }
        ...
        complete {
          ...
          db.run(getAgbotManagedDeploymentPatterns.result.transactionally.asTry).map {
            case Success(result) =>
              Future { logger.debug(s"GET /orgs/${organization}/agbots/${agreementBot}/patterns?deployment_pattern=${deploymentPattern}&deployment_pattern_organization=${deploymentPatternOrganization}&node_organization=${nodeOrganization} - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - result size: ${result.size}") }
  ...
  def postDeploymentPatterns(@Parameter(hidden = true) agreementBot: String,
    ...
    post {
      entity(as[PostAgbotPatternRequest]) {
        reqBody =>
          Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")})") }
          Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Request { nodeOrgid:${reqBody.nodeOrgid.getOrElse("None")}, pattern:${reqBody.pattern}, patternOrgid:${reqBody.patternOrgid} }") }
            validateWithMsg(reqBody.getAnyProblem) {
              ...
              complete({
                ...
                db.run(PatternsTQ.getPattern(OrgAndId(reqBody.patternOrgid, reqBody.pattern).toString).length.result.asTry
                                 .flatMap({
                                   case Success(num) =>
                                     Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Matched Deployment Patterns:         ${num}") }
                                     if (num > 0 ||
                                         reqBody.pattern == "*")
                                 ...
                                 .flatMap({
                                   case Success(v) => // Add the resource to the resourcechanges table
                                     Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Managed Deployment Patterns Created: ${v}") }
                  ...
                  .map({
                    case Success(v) =>
                      Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - Changes Logged:                      ${v}") }
                    ...
                    case Failure(exception: org.postgresql.util.PSQLException) =>
                      if (ExchangePosgtresErrorHandling.isDuplicateKeyError(exception)) {
                        Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${(HttpCode.ALREADY_EXISTS2, Serialization.write(ApiResponse(ApiRespType.ALREADY_EXISTS, ExchMsg.translate("pattern.foragbot.already.exists", deploymentPattern, resource))))}") }
                        ...
                        Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${(HttpCode.ACCESS_DENIED, Serialization.write(ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getMessage))))}") }
                        (HttpCode.ACCESS_DENIED, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getMessage)))
                      }
                      else {
                        Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${Serialization.write(ExchangePosgtresErrorHandling.ioProblemError(exception, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getServerErrorMessage)))}") }
                        ExchangePosgtresErrorHandling.ioProblemError(exception, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getServerErrorMessage))
                      }
                    case Failure(exception) =>
                      if (exception.getMessage.startsWith("Access Denied:")) {
                        ...
                        Future { logger.debug(s"POST /orgs/${organization}/agbots/${agreementBot}/patterns - ${identity.resource}:${identity.role}(${identity.identifier.getOrElse("")})(${identity.owner.getOrElse("")}) - ${exception.toString} - ${(HttpCode.BAD_INPUT, Serialization.write(ApiResponse(ApiRespType.BAD_INPUT, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getMessage))))}") }
                        (HttpCode.BAD_INPUT, ApiResponse(ApiRespType.BAD_INPUT, ExchMsg.translate("pattern.not.inserted", deploymentPattern, resource, exception.getMessage)))
                      }
                  })
              }) // end of complete
