...
import org.openhorizon.exchangeapi.route.node
import org.openhorizon.exchangeapi.table.{Node, _}
import org.openhorizon.exchangeapi.{Access, ApiRespType, ApiResponse, ApiTime, AuthCache, AuthRoles, AuthenticationSupport, ExchConfig, ExchMsg, ExchangePosgtresErrorHandling, HttpCode, IUser, Nth, OrgAndId, Password, StrConstants, TNode, table}
      ...
      validateWithMsg(GetNodesUtils.getNodesProblem(nodetype)) {
        complete({
          ...
          db.run(combinedQuery.result).map({ result =>
            ...
            val nodes: Map[String, table.Node] = result.map(e => e._1.id -> e._1.toNode(ident.isSuperUser, e._2)).toMap
            val code: StatusCode = if (nodes.nonEmpty) StatusCodes.OK else StatusCodes.NotFound
            (code, node.GetNodesResponse(nodes, 0))
          })
        }) // end of complete
      }
    } // end of exchAuth
          ...
          attribute match {
            ...
            case None =>   // Return the whole node
              ...
              db.run(q.result).map({ list =>
                ...
                if (list.nonEmpty) {
                  //val nodes = NodesTQ.parseJoin(ident.isSuperUser, list)
                  val nodes: Map[String, table.Node] = list.map(e => e._1.id -> e._1.toNode(ident.isSuperUser, e._2)).toMap
                  (HttpCode.OK, node.GetNodesResponse(nodes, 0))
                } else {
