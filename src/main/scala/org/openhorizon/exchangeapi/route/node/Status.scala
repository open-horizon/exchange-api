    ...
    complete({
      db.run(Compiled(NodeStatusTQ.getNodeStatus(resource)).result).map({
        list =>
          ...
          if(list.nonEmpty)
            (StatusCodes.OK, list.head.toNodeStatus) //response body
          else
            ...
            complete({
              db.run(reqBody.toNodeStatusRow(resource).upsert.asTry.flatMap({
                ...
                case Success(v) =>
                  ...
                  (StatusCodes.Created, ApiResponse(ApiRespType.OK, ExchMsg.translate("status.added.or.updated")))
                case Failure(t: org.postgresql.util.PSQLException) =>
                  if (ExchangePosgtresErrorHandling.isAccessDeniedError(t)) (StatusCodes.Forbidden, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("node.status.not.inserted.or.updated", resource, t.getMessage)))
                ...
                case Failure(t) =>
                  if (t.getMessage.startsWith("Access Denied:")) (StatusCodes.Forbidden, ApiResponse(ApiRespType.ACCESS_DENIED, ExchMsg.translate("node.status.not.inserted.or.updated", resource, t.getMessage)))
                  else (StatusCodes.InternalServerError, ApiResponse(ApiRespType.INTERNAL_ERROR, ExchMsg.translate("node.status.not.inserted.or.updated", resource, t.toString)))
              })
            })
          }
      }
        ...
        db.run(NodeStatusTQ.getNodeStatus(resource).delete.asTry.flatMap({
          case Success(v) =>
            ...
            if (v > 0) {
              ...
              DBIO.failed(new DBProcessingError(StatusCodes.NotFound, ApiRespType.NOT_FOUND, ExchMsg.translate("node.status.not.found", resource))).asTry
            }
          case Failure(t) => DBIO.failed(t).asTry
        })).map({
          case Success(v) => // there were no db status, but determine if it actually found it or not
            ...
            (StatusCodes.NoContent, ApiResponse(ApiRespType.OK, ExchMsg.translate("node.status.deleted")))
          case Failure(t: DBProcessingError) =>
            t.toComplete
          case Failure(t: org.postgresql.util.PSQLException) =>
            ExchangePosgtresErrorHandling.ioProblemError(t, ExchMsg.translate("node.status.not.deleted", resource, t.toString))
          case Failure(t) =>
            (StatusCodes.InternalServerError, ApiResponse(ApiRespType.INTERNAL_ERROR, ExchMsg.translate("node.status.not.deleted", resource, t.toString)))
        })
      })
    }
