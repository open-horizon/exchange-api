...
class NodeStatuses(tag: Tag) extends Table[NodeStatusRow](tag, "nodestatus") {
  ...
  def node: ForeignKeyQuery[Nodes, NodeRow] = foreignKey("node_status_fk_node", nodeId, NodesTQ)(_.id, onUpdate = ForeignKeyAction.Cascade, onDelete = ForeignKeyAction.Cascade)
