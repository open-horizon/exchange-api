...
source `dirname $0`/functions.sh $2 ${@:4}

resource=${3#/}     # remove leading slash in case there, because we will add it below
...
curl $copts -X $method -H 'Accept: application/json' -H "type:$idType" -H "id:$id" -H "orgid:$EXCHANGE_ORG" -H "issuer:IBM_ID" $EXCHANGE_URL_ROOT/v1/orgs/$EXCHANGE_ORG/$resource | $parse