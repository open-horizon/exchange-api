...
import com.horizon.exchangeapi.{ApiTime, ApiUtils, HttpCode, MaxChangeIdResponse, Password, Role, TestDBConnection}
...
class TestGetMaxChangeIDRoute extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("GET /changes/maxchangeid -- as root -- success") {
    val maxChangeIdBeforeRequest: Long = Await.result(DBCONNECTION.getDb.run(ResourceChangesTQ.sortBy(_.changeId.desc).take(1).result), AWAITDURATION).head.changeId
