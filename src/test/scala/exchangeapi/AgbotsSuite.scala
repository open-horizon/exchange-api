...
class AgbotsSuite extends FunSuite {
  ...
  /** Delete all the test agbots and agreements, in case they exist from a previous run
  test("Begin - DELETE all test agbots and agreements") {
    deleteAllAgreements
    deleteAllAgbots
  }
  */
  ...
  test("PUT /agbots/"+agbotId+" - normal") {
    val input = PutAgbotsRequest(agbotToken, "agbot"+agbotId+"-norm", "whisper-id")
  ...
  test("PUT /agbots/"+agbotId+" - normal - as user") {
    val input = PutAgbotsRequest(agbotToken, "agbot"+agbotId+"-normal-user", "whisper-id")
    val response = Http(URL+"/agbots/"+agbotId).postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.PUT_OK)
  }
  ...
  test("PUT /agbots/"+agbotId+" - normal - as agbot") {
    val input = PutAgbotsRequest(agbotToken, "agbot"+agbotId+"-normal", "whisper-id")
  ...
  test("PUT /agbots/9932 - bad creds") {
    val input = PutAgbotsRequest("mytok", "agbot9932-badcreds", "whisper-id")
  ...
  test("GET /agbots/"+agbotId+" - as agbot") {
    val response: HttpResponse[String] = Http(URL+"/agbots/"+agbotId).headers(ACCEPT).headers(AGBOTAUTH).asString
    info("code: "+response.code)
    // info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.OK)
    val getAgbotResp = parse(response.body).extract[GetAgbotsResponse]
  ...
  test("PUT /agbots/"+agbot2Id+" - normal") {
    val input = PutAgbotsRequest(agbot2Token, "agbot"+agbot2Id+"-norm", "whisper-id")
  ...
  test("PUT /agbots/"+agbotId+"/agreements/9950 - as agbot") {
    val input = PutAgbotAgreementRequest("sdr", "signed")
    val response = Http(URL+"/agbots/"+agbotId+"/agreements/9950").postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(AGBOTAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.PUT_OK)
  }

  /** Update the agreement for agbot 9930 - as user */
  test("PUT /agbots/"+agbotId+"/agreements/9950 - as user") {
    val input = PutAgbotAgreementRequest("sdr", "negotiating")
    val response = Http(URL+"/agbots/"+agbotId+"/agreements/9950").postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.PUT_OK)
  }
  ...
  test("GET /agbots/"+agbotId+"/agreements/9950") {
    val response: HttpResponse[String] = Http(URL+"/agbots/"+agbotId+"/agreements/9950").headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code)
    assert(response.code === HttpCode.OK)
    val getAgResp = parse(response.body).extract[GetAgbotAgreementsResponse]
    assert(getAgResp.agreements.size === 1)
    ...
    info("GET /agbots/"+agbotId+"/agreements/9950 output verified")
  }

  test("GET /agbots/"+agbotId+"/agreements/9950 - as agbot") {
    val response: HttpResponse[String] = Http(URL+"/agbots/"+agbotId+"/agreements/9950").headers(ACCEPT).headers(AGBOTAUTH).asString
    info("code: "+response.code)
    assert(response.code === HttpCode.OK)
    val getAgResp = parse(response.body).extract[GetAgbotAgreementsResponse]
    assert(getAgResp.agreements.size === 1)
