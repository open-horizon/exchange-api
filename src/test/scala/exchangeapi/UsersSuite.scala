...
/**
 * Tests for the /orgs/"+orgid+"/users routes. To run
...
class UsersSuite extends FunSuite {
  ...
  def deleteAllUsers() = {
    for (i <- List(user)) {     // we do not delete the root user because it was created by the config file, not this test suite
      val response = Http(URLORG+"/users/"+i).method("delete").headers(ACCEPT).headers(ROOTAUTH).asString
  ...
  test("GET number of existing users") {
    val response: HttpResponse[String] = Http(URLORG+"/users").headers(ACCEPT).headers(ROOTAUTH).asString
  ...
  test("POST /orgs/"+orgid+"/users/"+user+" - bad format") {
    ...
    val response = Http(URLORG+"/users/"+user).postData(badJsonInput).method("post").headers(CONTENT).headers(ACCEPT).headers(ROOTAUTH).asString
    info("code: "+response.code)
  ...
  test("POST /orgs/"+orgid+"/users/"+user+" - no email") {
    ...
    val response = Http(URLORG+"/users/"+user).postData(write(input)).method("post").headers(CONTENT).headers(ACCEPT).headers(ROOTAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
  ...
  test("PUT /orgs/"+orgid+"/users/"+user+" - update normal") {
    ...
    val response = Http(URLORG+"/users/"+user).postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
  ...
  test("PUT /orgs/"+orgid+"/users/"+user+" - update normal - as root") {
    val input = PutUsersRequest(pw, user+"@gmail.com")
    val response = Http(URLORG+"/users/"+user).postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(ROOTAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.PUT_OK)
  }
  ...
  test("GET /orgs/"+orgid+"/users - as "+user) {
    val response: HttpResponse[String] = Http(URLORG+"/users").headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code)
    // info("code: "+response.code+", response.body: "+response.body)
  ...
  test("GET /orgs/"+orgid+"/users/"+user) {
    val response: HttpResponse[String] = Http(URLORG+"/users/"+user).headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code)
    // info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.OK)
    val getUserResp = parse(response.body).extract[GetUsersResponse]
  ...
  test("PUT /orgs/"+orgid+"/users/"+user+" - update normal with encoded creds") {
    ...
    val response = Http(URLORG+"/users/"+user).postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(ENCODEDAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.PUT_OK)
  }
  ...
  test("GET /orgs/"+orgid+"/users/"+user+" - encoded creds") {
    val response: HttpResponse[String] = Http(URLORG+"/users/"+user).headers(ACCEPT).headers(ENCODEDAUTH).asString
    info("code: "+response.code)
    // info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.OK)
    val getUserResp = parse(response.body).extract[GetUsersResponse]
  ...
  test("POST /orgs/"+orgid+"/users/"+user+"/confirm") {
    val response = Http(URLORG+"/users/"+user+"/confirm").method("post").headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.POST_OK)
  ...
  test("POST /orgs/"+orgid+"/users/"+user+"/confirm - encoded") {
    ...
    val response = Http(URLORG+"/users/"+user+"/confirm").method("post").headers(ACCEPT).headers(ENCODEDAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.POST_OK)
    val postConfirmResp = parse(response.body).extract[ApiResponse]
    assert(postConfirmResp.code === ApiResponseType.OK)
  ...
  test("POST /orgs/"+orgid+"/users/"+user+"/confirm - bad pw") {
    val response = Http(URLORG+"/users/"+user+"/confirm").method("post").headers(ACCEPT).headers(("Authorization","Basic "+user+":badpw")).asString
    info("code: "+response.code+", response.body: "+response.body)
  ...
  test("POST /orgs/"+orgid+"/users/"+user+"/changepw") {
    ...
    var response = Http(URLORG+"/users/"+user+"/reset").method("post").headers(ACCEPT).headers(ROOTAUTH).option(CONNTIMEOUT).option(READTIMEOUT).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.POST_OK)
    ...
    response = Http(URLORG+"/users/"+user+"/changepw").postData(write(input)).headers(CONTENT).headers(ACCEPT).headers(TOKENAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.POST_OK)
    ...
    response = Http(URLORG+"/users/"+user+"/confirm").method("post").headers(ACCEPT).headers(NEWAUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
    assert(response.code === HttpCode.POST_OK)
    val postConfirmResp = parse(response.body).extract[ApiResponse]
    assert(postConfirmResp.code === ApiResponseType.OK)
    ...
    response = Http(URLORG+"/users/"+user+"/reset").method("post").headers(ACCEPT).headers(ROOTAUTH).asString
    info("code: "+response.code)
    assert(response.code === HttpCode.POST_OK)
  ...
  test("POST /orgs/"+orgid+"/users/"+user+"/reset - request email") {
    ...
    if (spamEmail != null) {
      ...
      var response = Http(URLORG+"/users/"+user).postData(write(input)).method("put").headers(CONTENT).headers(ACCEPT).headers(AUTH).asString
      info("code: "+response.code+", response.body: "+response.body)
      assert(response.code === HttpCode.PUT_OK)

      // verify the email was set and the pw was not
      response = Http(URLORG+"/users/"+user).headers(ACCEPT).headers(AUTH).asString
      info("code: "+response.code)
      // info("code: "+response.code+", response.body: "+response.body)
      assert(response.code === HttpCode.OK)
      val getUserResp = parse(response.body).extract[GetUsersResponse]
      ...
      response = Http(URLORG+"/users/"+user+"/reset").method("post").headers(ACCEPT).option(CONNTIMEOUT).option(READTIMEOUT).asString    // Note: no AUTH
      info("code: "+response.code+", response.body: "+response.body)
      assert(response.code === HttpCode.POST_OK)
  ...
  test("DELETE /orgs/"+orgid+"/users/"+user) {
    val response = Http(URLORG+"/users/"+user).method("delete").headers(ACCEPT).headers(AUTH).asString
    info("code: "+response.code+", response.body: "+response.body)
